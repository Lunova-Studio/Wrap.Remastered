...
/// P2P连接拒绝包处理器
/// </summary>
public class PeerConnectRejectPacketHandler : BasePacketHandler
{
    public PeerConnectRejectPacketHandler(IWrapServer server) : base(server)
    {
    }
    ...
    public override async Task OnHandleAsync(IChannel channel, UnsolvedPacket packet)
    {
        try
        {
            var rejectPacket = PeerConnectRejectPacket.Serializer.Deserialize(packet.Data) as PeerConnectRejectPacket;
            if (rejectPacket == null)
            {
                Server.GetLoggingService().LogError("Packet", "P2P连接拒绝包反序列化失败", null, "通道: {0}", channel.RemoteAddress);
                return;
            }
