...
public class KickCommand : CommandBase, ICommandTabCompleter {
    ...
    public KickCommand(IServerCoordinator server) {
    ...
    public override async Task OnExecuteAsync(string[] args) {
        ...
        var success = await _server.GetConnectionManager()
            .DisconnectUserAsync(userId, reason);
    ...
    public IEnumerable<string> OnComplete(string[] args) {
        ...
        if (args.Length == 1) {
            // 优先补全在线用户ID
            var connections = _server.GetConnectionManager()
                .GetAllUserConnections();

            list.AddRange(connections.Where(c => c.UserInfo != null)
