...
public class PeerConnectAcceptPacketHandler : BasePacketHandler {
    public PeerConnectAcceptPacketHandler(IServerCoordinator server) : base(server) {
    }

    public override async Task OnHandleAsync(IChannel channel, UnsolvedPacket packet) {
        try {
            // 反序列化P2P连接接受包
            if (PeerConnectAcceptPacket.Serializer.Deserialize(packet.Data) is not PeerConnectAcceptPacket peerConnectAcceptPacket) {
                Server.Logger.LogError(new ArgumentNullException(nameof(peerConnectAcceptPacket)),
                    "P2P 连接接受包反序列化失败, 通道: {RemoteAddress}", channel.RemoteAddress);
