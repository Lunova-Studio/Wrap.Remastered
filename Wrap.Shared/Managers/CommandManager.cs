...
namespace Wrap.Shared.Managers;

public sealed class CommandManager {
    private readonly List<ICommandExecuter> _executors = [];
    private readonly List<ICommandTabCompleter> _tabCompleters = [];
    ...
    public IEnumerable<string> Complete(string command, string[] args) {
    ...
    /// <returns>补全选项列表</returns>
    public IEnumerable<string> Complete(string commandLine) {
        ...
        // 主命令补全
        if (!trimmed.Contains(' '))
            return [.. GetAllCommands().Where(c => c.StartsWith(trimmed, StringComparison.OrdinalIgnoreCase))];
    ...
    public IEnumerable<string> GetAllCommands() {
        return [.. _executors.Select(x => x.GetName())];
    }
